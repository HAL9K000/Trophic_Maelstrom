...
int main(int argc, char* argv[])
{
  ...
  for (int s = 0; s < SpB; s++)
  {
    ...
    std::cerr << "For species " << s << " with [D, v, dtv]: " << D[s] << ", " << v[s] << ", " << dt_adv[s] << ", we have:\n";
    std::cerr << "Beta_V = " << beta_vel[s] << "\t sigma_D = " << sigma_D[s] << " with scaled bound = " << sig_D_ScaledBounds[s]
      << "\t sigma_vD( X, Y) = { " << sigma_vD[s].first << ", " << sigma_vD[s].second << " } with scaled bounds = { "
      << sig_vD_ScaledBounds[s].first << ", " << sig_vD_ScaledBounds[s].second << " } and AVG Shift mu : " << mu_vel_prefactor[s] * v[s] << "\n";
    if (sigma_vD[s].first == sigma_vD[s].second)
    {
      cout << "Additionally velocity OU Process for species " << s << " IS SYMMETRIC.\n";
      std::cerr << "Additionally velocity OU Process for species " << s << " IS SYMMETRIC.\n";
    }
    else {
      cerr << "Additionally velocity OU Process for species " << s << " IS NOT SYMMETRIC.\n";
      std::cout << "Additionally velocity OU Process for species " << s << " IS NOT SYMMETRIC.\n";
    }
  ...
  cerr << "Values of dtV for the grazer and predator are: " << dtV[1] << " and " << dtV[2] << " respectively.\n";
