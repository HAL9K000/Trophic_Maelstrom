...
#ifdef LOCUST_WIND
	#include "rietkerk_locust_case.h"
#endif
...
void calc_gamma_3Sp(const vector<pair<int, int>>& centralNeighboringSites, D2Vec_Double &Rho_t, vector <double> &gamma_i,  
			double (&Rho_avg)[Sp], vector <std::pair<double, int>>& rfrac, vector <std::pair<int, int>>& dtV_counter,  double nVeg_frac, int r_max, int i, int L);
...
void rietkerk_Dornic_2D_MultiSp(D2Vec_Double &Rho, vector <double> &t_meas, double t_max, double a, double c, double gmax, double alpha, double rW, double W0, 
	...
	int (&dtV)[SpB], double clow[], double dt, double dx, double dP, int r, int g, FFTW3_CentralPlanner& fft_central_plan, double Gstar  = -1.0,  double Vstar = -1.0 );
